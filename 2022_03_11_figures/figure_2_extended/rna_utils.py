...
def index_to_nucleotide() -> Dict:
    '''Returns one hot indices of nucleotides in data.'''
    return {0: 'A', 1: 'C', 2: 'G', 3: 'U'}

def nucleotide_to_index() -> Dict:
    return {v: k for k, v in index_to_nucleotide().items()}

def index_to_structure() -> Dict:
    '''Returns one hot indices of structure characters in data.'''
    return {0: '.', 1: '(', 2: ')'}


def RNAsubopt(seq: str, RNAsubopt_bin: str = "RNAsubopt", delta_energy: float = 5.0) -> List[Tuple[str, float]]:
    """
    ...
    try:
        ...
        out = (
            subprocess.check_output(
                [RNAfold_bin, f"--stochBT={num_structs}", f"-T {temperature}"]
...
def RNAfold(
    ...
    try:
        ...
        out = (
            subprocess.check_output(
                [RNAfold_bin, "-j 8", "--noPS", f"-T {temperature}"]
...
def RNA_partition_function(
    ...
    try:
        input_binary = b"\n".join(
            [(seqs[i] + "\n" + constraints[i]).encode('ascii') for i in range(len(seqs))]
        )  # convert to binary

        out = (
            subprocess.check_output(
                [RNAfold_bin, "-j 8", "--noPS", "--noDP", "-p0", "-C", f"-T {temperature}"]
                + (["--commands=" + commands_file] if commands_file != "" else []),
                input=input_binary,
            )
            .decode()
        ...
        ret_value = [
            float(out[i].split(sep=" ")[-2]) for i in range(2, len(out), 4)
        ]  # take the third line in each group of 4 lines
